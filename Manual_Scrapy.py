...
N       = int(config['DEFAULT']['ahead'].strip())
...
def csv_merge():
  ...
  for case_type in case_type_list:
    ...
    file      = open(os.path.join(b'./Excel',file_name), "rb")
...
def start():
  ...
  for case_type in case_type_list:
    if case_type == 'general':
      ...
      cmd=("libreoffice --calc " + "Stanley_Mosk_Courthouse_"+ str(current_time) + "civil.csv")
      print(cmd)
      #print(type(file_name))
    else:
      file_name = ("Stanley_Mosk_Courthouse_"+ str(current_time)+case_type+ ".csv").encode('utf-8')
    directory_in_str = b'./Excel/'+file_name
